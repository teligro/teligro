...
class WPStatistics extends Teligro {
	...
	function send_report( $final_text_report ) {
		...
		$text_report = Helpers::br2nl( $text_report );
		$text_report = wp_strip_all_tags( $text_report );
		$text        .= $text_report . "\n";

		$text .= __( 'Date', $this->plugin_key ) . ': ' . Helpers::localeDate() . "\n";
		$text = apply_filters( 'teligro_wpstatistics_send_report_notification_text', $text, $final_text_report );
	...
	static function getInstance() {
		if ( self::$instance == null )
			self::$instance = new WPStatistics();

		return self::$instance;
	}
}

WPStatistics::getInstance();