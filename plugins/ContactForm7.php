...
class ContactForm7 extends Teligro {
	protected $cf7_fields = array( 'email', 'name', 'subject', 'message' ),
		$cf7_prefix_field = 'teligro_',
	...
	function wpcf7_submit( $contact_form, $result ) {
		...
		if ( 'mail_sent' == $result['status'] ) {
			...
			if ( $users ) {
				...
				if ( $message ) {
					$message = apply_filters( 'teligro_wpcf7_notification_message', $message, $args,
						$contact_form, $result );
				...
				$text = apply_filters( 'teligro_wpcf7_message_notification_text', $text, $args, $contact_form,
					$result );
	...
	function wpcf7_get_value( $field, $contact_form ) {
		...
		$value = apply_filters( 'teligro_wpcf7_get_value', $value,
			$field, $contact_form );
	...
	static function getInstance() {
		if ( self::$instance == null )
			self::$instance = new ContactForm7();
