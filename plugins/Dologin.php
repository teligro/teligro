...
class DoLogin extends Teligro {
	public static $instance = null;
	private $_dry_run = false;

	public function __construct() {
		parent::__construct();
		add_action( 'teligro_plugins_settings_content', [ $this, 'settings_content' ] );
		...
		if ( $this->get_option( 'dologin_plugin_two_factor_auth', false ) ) {
			...
			add_filter( 'teligro_localize_script', [ $this, 'localize_script' ], 30, 3 );
	...
	function settings_content() {
        ...
        <tr>
            <td>
                <label for="dologin_plugin_two_factor_auth"><?php _e( 'Two factor auth with Telegram bot',
						$this->plugin_key ) ?></label>
            </td>
            <td>
                <label><input type="checkbox" value="1" id="dologin_plugin_two_factor_auth"
                              name="dologin_plugin_two_factor_auth" <?php checked( $this->get_option( 'dologin_plugin_two_factor_auth',
        ...
        <tr>
            <td>
                <label for="dologin_plugin_force_two_factor_auth"><?php _e( 'Force Telegram bot auth validation',
						$this->plugin_key ) ?></label>
            </td>
            <td>
                <label><input type="checkbox" value="1" id="dologin_plugin_force_two_factor_auth"
                              name="dologin_plugin_force_two_factor_auth" <?php checked( $this->get_option( 'dologin_plugin_force_two_factor_auth',
						0 ), 1 ) ?>> <?php _e( 'Active', $this->plugin_key ) ?>
                </label>
                <p class="description">
	...
	public function send() {
		...
		// Expire old ones
		$wpdb->query( $wpdb->prepare( "UPDATE $tb_sms SET used = -1 WHERE user_id = %d AND used = 0",
			array( $user->ID ) ) );
		...
		$q = 'INSERT INTO ' . $tb_sms . ' ( ' . implode( ',', array_keys( $s ) ) . ' ) VALUES ( ' . implode( ',',
				array_fill( 0, count( $s ), '%s' ) ) . ' )';
		...
		// Update log
		$wpdb->query( $wpdb->prepare( "UPDATE $tb_sms SET res = %s WHERE id = %d",
			array( json_encode( $result ), $id ) ) );

		// Expected response
		if ( $result && isset( $result['ok'] ) && $result['ok'] ) {
			$usernameStar = Helpers::string2Stars( $bot_user['username'] );
			$message      = sprintf( __( 'Sent dynamic code to this Telegram account @%s.', $this->plugin_key ),
				$usernameStar );
	...
	function send_notification( $bot_user, $message, $user ) {
		if ( $bot_user ) {
			...
			$text .= $message;
			$text = apply_filters( 'teligro_dologin_plugin_two_factor_auth_notification_text', $text, $bot_user,
				$message, $user );
