...
class Wordfence extends Teligro {
	...
	public function __construct() {
		...
		add_filter( 'teligro_words', [ $this, 'new_words' ] );
	...
	function event_action( $event, $data, $alertCallback = null ) {
		...
		foreach ( $data as $key => $value ) {
            ...
            elseif ( $label == 'username' )
				$label = 'userName';
			$label = Helpers::wordsCamelCaseToSentence( $label );

			if ( $key == 'duration' ) {
				$value = Helpers::secondsToHumanTime( $value,
					array(
				...
				foreach ( $value as $row ) {
					$attackTable .= Helpers::localeDate( $row['date'] ) . ' | ' . $row['IP'] . ' | ' . $row['country'] . ' | ' . $row['message'] . "\n";
				}
				$value = '';
		...
		$text = apply_filters( 'teligro_wordfence_security_event_notification_text', $text, $event, $data,
			$alertCallback );
	...
	static function getInstance() {
		if ( self::$instance == null )
			self::$instance = new Wordfence();

		return self::$instance;
	}
}

Wordfence::getInstance();