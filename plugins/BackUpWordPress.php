...
class BackUpWordPress extends Teligro {
	...
	public function __construct() {
		...
		if ( $this->get_option( 'backupwordpress_plugin_new_backup_notification', false ) ) {
			add_action( 'teligro_backupwordpress_plugin_new_backup', [ $this, 'new_backup' ], 10, 2 );
	...
	function plugins_loaded() {
		require_once TELIGRO_PLUGINS_DIR . 'BackUpWordPressService.php';
	}
	...
	function new_backup( $backup, $attacheFile ) {
		...
		if ( ! file_exists( $file ) && ( $errors = array_merge( $backup->get_errors(), $backup->get_warnings() ) ) ) {
			...
			if ( $attacheFile ) {
				...
				$maxFileSize = wp_convert_hr_to_bytes( TELIGRO_MAX_FILE_SIZE );
	...
	protected function send_notification( $title, $message, $backup, $file = false ) {
		$text = $title . "\n\n";
		$text .= $message;
		$text .= "\n" . __( 'Date', $this->plugin_key ) . ': ' . Helpers::localeDate() . "\n";
		$text = apply_filters( 'teligro_backupwordpress_plugin_new_backup_notification_text', $text, $title,
			$message, $backup );
	 ...
	 * @return BackUpWordPress
	 */
	static function getInstance() {
		if ( self::$instance == null )
			self::$instance = new BackUpWordPress();

		return self::$instance;
	}
}

BackUpWordPress::getInstance();