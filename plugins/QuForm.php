...
use Quform as Quform_Main;
use Quform_Repository;

if ( ! defined( 'ABSPATH' ) )
	exit;

class QuForm extends Teligro {
	...
	function form_submit( $result, $form ) {
		...
		foreach ( $form->getRecursiveIterator() as $element ) {
			if ( $element instanceof Quform_Element_Field && $element->config( 'saveToDatabase' ) ) {
				...
				$fieldsText .= Quform_Main::escape( $element->getLabel() ) . ': ';
		...
		$text = apply_filters( 'teligro_quform_message_notification_text', $text, $form );
