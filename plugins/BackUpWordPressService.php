 ...
 * @extends Service
 */
class BackUpWordPressService extends Service {
	/**
	 * Human readable name for this service
	 * @var string
	 */
	public $name = 'Teligro Integration';
	...
	public function field() {
        ...
        <tr>
            <th scope="row">
                <label for="<?php echo esc_attr( $this->get_field_name( 'notification' ) ); ?>"><?php _e( 'Telegram',
						TELIGRO_PLUGIN_KEY ); ?></label>
            </th>
            <td>
                <label><input type="checkbox" value="1"
                              id="<?php echo esc_attr( $this->get_field_name( 'notification' ) ); ?>"
                              name="<?php echo esc_attr( $this->get_field_name( 'notification' ) ); ?>" <?php checked( $this->get_field_value( 'notification' ),
						1 ) ?>> <?php _e( 'Notification', TELIGRO_PLUGIN_KEY ) ?>
                </label><br>
                <label><input type="checkbox" value="1"
                              id="<?php echo esc_attr( $this->get_field_name( 'attache' ) ); ?>"
                              name="<?php echo esc_attr( $this->get_field_name( 'attache' ) ); ?>" <?php checked( $this->get_field_value( 'attache' ),
						1 ) ?>> <?php _e( 'Attache backup file', TELIGRO_PLUGIN_KEY ) ?>
                </label>
            </td>
        </tr>
	<?php }
	...
	public function display() {
		if ( $this->is_service_active() )
			return __( 'Telegram', TELIGRO_PLUGIN_KEY );
	...
	public function action( $action, Backup $backup ) {
		if ( 'hmbkp_backup_complete' === $action && $this->is_service_active() )
			do_action( 'teligro_backupwordpress_plugin_new_backup', $backup, $this->is_attache_active() );
	}
...
Services::register( __FILE__, 'teligro\BackUpWordPressService' );