...
class Channel extends Teligro {
		...
		if ( 'telegram-teligro' === $column ) {
			...
			foreach ( $options['channel_username'] as $k => $v ) {
				...
				if ( in_array( $post_type, $post_types ) )
					echo '<span class="dashicons-teligro-telegram quick-send-channel-teligro" title="' . __( 'Send to Channel',
							$this->plugin_key ) . ': @' . $v . '" data-channel="' . $v . '" data-id="' . $post_id . '" data-index="' . $k . '"></span>';
			}
		}
	}
	...
	function channel_init() {
		...
		if ( is_admin() && $pagenow == 'edit.php' && in_array( $post_type,
				$postTypes ) && $this->check_metabox_display() && $this->get_option( 'quick_send_to_channel',
				false ) ) {
	...
	function helps_channel() {
        ...
        <div class="item">
            ...
            <div class="panel">
                <div>
                    <strong>
						<?php _e( 'Telegram channel is a great way for attracting people to your site.<br> This option allows you to send posts to your Telegram channel.',
							$this->plugin_key ) ?>
                    </strong>
                    <ol>
                        ...
                        <li><?php _e( "Go to channel options and select 'Administrator' option.",
								$this->plugin_key ) ?></li>
                        <li><?php _e( "Select 'Add Administrator' option.", $this->plugin_key ) ?></li>
                        <li><?php _e( "Search the username of your bot and add it as administrator.",
								$this->plugin_key ) ?></li>
                        <li><?php _e( "Allow 'Post Messages' Permission", $this->plugin_key ) ?></li>
                        <li><?php _e( "Enter the username of the channel and hit SAVE button!!!",
								$this->plugin_key ) ?></li>
                        <li><?php _e( "Yes! Now, whenever you publish or update a post you can choose whether send it to Telegram (from post editor page)",
								$this->plugin_key ) ?>
                        </li>
                    </ol>
                </div>
	...
	function replace_channel_text( $template, $post_id ) {
			...
			foreach ( $matches[0] as $field ) {
				...
				if ( $_field[0] == 'cf' ) {
					...
					if ( taxonomy_exists( $_field[1] ) ) {
						...
						$names = ( is_wp_error( $terms ) || empty( $terms ) ) ? array() : wp_list_pluck( $terms,
							'name' );
						if ( ! empty( $names ) ) {
		 ...
		 * Check if statement
		 */
		$template = preg_replace( '/{if=\'(.*?)\'}(.*?){\/if}/',
			'[if_statement_teligro field="$1" post_id="' . $post_id . '"]$2[/if_statement_teligro]', $template );
	...
	function send_to_channel( $post_id, $channel, $index = 0 ) {
		...
		if ( $options['channel_username'][ $index ] == $channel ) {
			...
			$text = get_post_meta( $post_id,
				'_channel_message_pattern_' . $options['channel_username'][ $index ] . '_teligro', true );
			if ( empty( $text ) )
			...
			if ( isset( $options['channel_inline_button_title'][ $index ] ) && ! empty( $options['channel_inline_button_title'][ $index ] ) ) {
				$inlineKeyboards = array(
					array(
						array(
							'text' => $options['channel_inline_button_title'][ $index ],
							'url'  => $post['short-link']
						)
					)
				);
			...
			if ( $post['image'] !== null ) {
				$featured_image = get_post_meta( $post_id,
					'_featured_image_' . $options['channel_username'][ $index ] . '_teligro', true );
	...
	function schedule() {
		if ( isset( $this->options['send_to_channels'] ) && ! wp_next_scheduled( 'auto_channels_teligro' ) )
			wp_schedule_event( current_time( 'U' ),
				'every_' . ( intval( $this->options['channels_cron_interval'] ) != 0 ? $this->options['channels_cron_interval'] : 1 ) . '_minutes',
	...
	function meta_save( $post_id, $post ) {
		...
		foreach ( $options['channel_username'] as $k => $v ) {
			if ( empty( $v ) || ! isset( $options['channel_post_type'][ $k ] ) || isset( $options['channel_post_type'][ $k ] ) && is_array( $options['channel_post_type'][ $k ] ) && ! in_array( $post_type,
					array_keys( $options['channel_post_type'][ $k ] ) ) )
				continue;

			if ( $display_metabox ) {
				$posted_status = get_post_meta( $post_id,
					'_retry_posted_' . $options['channel_username'][ $k ] . '_teligro', true );
				...
				if ( ! empty( $_POST['channel_message_pattern'][ $options['channel_username'][ $k ] ] ) && stripslashes( $options['channel_message_pattern'][ $k ] ) != $_POST['channel_message_pattern'][ $options['channel_username'][ $k ] ] )
					update_post_meta( $post_id,
						...
						sanitize_textarea_field( $_POST['channel_message_pattern'][ $options['channel_username'][ $k ] ] ) );
			...
			update_post_meta( $post_id, '_send_to_channel_' . $options['channel_username'][ $k ] . '_teligro',
				$send_to_channel );
	...
	function post_display() {
        ...
        <div class="wrap teligro-metabox channel-list-teligro accordion-teligro">
			...
			foreach ( $options['channel_username'] as $k => $v ) {
				...
				$post_types = array_keys( $options['channel_post_type'][ $k ] );
				if ( empty( $v ) || in_array( $options['channel_username'][ $k ],
						$current_channel ) || ! isset( $options['channel_post_type'][ $k ] ) || isset( $options['channel_post_type'][ $k ] ) && is_array( $options['channel_post_type'][ $k ] ) && ! in_array( get_post_type( $post_id ),
						$post_types ) )
					continue;
				$current_channel[] = $options['channel_username'][ $k ];
				$send_to_channel   = get_post_meta( $post_id,
					'_send_to_channel_' . $options['channel_username'][ $k ] . '_teligro', true );

				if ( empty( $send_to_channel ) )
					$send_to_channel = isset( $options['send_to_channel'][ $k ] ) ? 1 : 2;

				$channel_message_pattern = get_post_meta( $post_id,
					'_channel_message_pattern_' . $options['channel_username'][ $k ] . '_teligro', true );
				...
				$featured_image = get_post_meta( $post_id,
					'_featured_image_' . $options['channel_username'][ $k ] . '_teligro', true );
				if ( empty( $featured_image ) )
					$featured_image = isset( $options['channel_with_featured_image'][ $k ] ) ? 1 : 2;

				$posted_status = get_post_meta( $post_id,
					'_posted_status_' . $options['channel_username'][ $k ] . '_teligro', true );
				$posted        = '';
                                    ...
                                    <fieldset>
                                        <label>
                                            <input type="radio" class="send-to-channel send-to-channel-yes"
                                                   value="1" <?php checked( isset( $send_to_channel ) ? $send_to_channel : '',
												1 ) ?>
                                                   name="send_to_channel[<?php echo $options['channel_username'][ $k ] ?>]"> <?php _e( 'Yes',
												$this->plugin_key ) ?>
                                        </label>
                                        <label>
                                            <input type="radio" class="send-to-channel send-to-channel-no"
                                                   value="2" <?php checked( isset( $send_to_channel ) ? $send_to_channel : '',
												2 ) ?>
                                                   name="send_to_channel[<?php echo $options['channel_username'][ $k ] ?>]"> <?php _e( 'No',
												$this->plugin_key ) ?>
                                        </label>
                                    </fieldset>
                                </td>
                            ...
                            <tr>
                                ...
                                <td>
                                    <fieldset>
                                        <label>
                                            <input type="radio"
                                                   value="1" <?php checked( isset( $featured_image ) ? $featured_image : '',
												1 ) ?>
                                                   name="channel_with_featured_image[<?php echo $options['channel_username'][ $k ] ?>]"> <?php _e( 'Yes',
												$this->plugin_key ) ?>
                                        </label>
                                        <label>
                                            <input type="radio"
                                                   value="2" <?php checked( isset( $featured_image ) ? $featured_image : '',
												2 ) ?>
                                                   name="channel_with_featured_image[<?php echo $options['channel_username'][ $k ] ?>]"> <?php _e( 'No',
												$this->plugin_key ) ?>
                                        </label>
                                    </fieldset>
                                </td>
	...
	function channel_members_count() {
		...
		if ( $channel && $channel_member['ok'] && isset( $channel_member['result'] ) ) {
			...
			_e( 'API Token or Channel Username Invalid, It may not have access to the channel! (Read Help)',
				$this->plugin_key );
		}
		exit;
	}
                    ...
                    <td>
                        <input type="number" name="channels_cron_interval" id="channels_cron_interval"
                               ...
                               class="small excerpt_length ltr"> <?php _e( 'Minutes', $this->plugin_key ) ?><span
                                class="description">,
                            <?php printf( __( 'Work with <a href="%s" target="_blank">WP-Cron</a>.',
	                            $this->plugin_key ),
                ...
                <tr>
                    ...
                    <td>
                        <input type="number" name="channels_delay_send" id="channels_delay_send"
                               value="<?php echo $this->get_option( 'channels_delay_send', 5 ) ?>" placeholder="0"
                               min="0"
                               class="small excerpt_length ltr"> <?php _e( 'Minutes', $this->plugin_key ) ?><span
                                class="description">, <?php _e( 'Least 5 minutes recommended.',
                ...
                <tr>
                    ...
                    <td>
						...
						foreach ( $user_roles as $role => $name )
							echo '<label><input type="checkbox" name="channels_metabox_user_roles[' . $role . ']" value="1" ' . checked( isset( $channels_user_roles[ $role ] ),
									true, false ) . '>' . $name . '</label>'
						?>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        <span class="description"><?php _e( 'Display Telegram column in admin post type archive (Requires the metabox display access).',
								$this->plugin_key ) ?></span>
                    </td>
                </tr>
            </table>
	...
	private function item( $item ) {
        ...
        <div class="item" data-index="<?php echo $item['index'] ?>">
            <button class="toggle"
                    type="button"><?php echo isset( $item['channel_username'] ) && ! empty( $item['channel_username'] ) ? '@' . $item['channel_username'] : __( 'New Channel',
					$this->plugin_key ); ?></button>
            <div class="panel">
                <table>
                    <tr>
                        <td>
                    ...
                    <tr>
                        ...
                        <td>
                            <label><input type="checkbox" class="send_to_channel"
                                          value="1" <?php checked( isset( $item['send_to_channel'] ) ? $item['send_to_channel'] : '',
									1 ) ?>
                                          name="send_to_channel[<?php echo $item['index'] ?>]"> <?php _e( 'Active',
									$this->plugin_key ) ?>
                            </label>
                        </td>
                    </tr>
                        ...
                        <td>
							...
							foreach ( $this->post_types as $post_type ) {
								if ( ! in_array( $post_type->name, $this->ignore_post_types ) )
									echo '<label><input type="checkbox" name="channel_post_type[' . $item['index'] . '][' . $post_type->name . ']" value="1" ' . checked( isset( $item['channel_post_type'][ $post_type->name ] ) ? $item['channel_post_type'][ $post_type->name ] : 0,
											1,
											false ) . ' class="channel_post_type"/>' . $post_type->label . '</label> &nbsp;';
							}
							?>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <label><input type="checkbox" class="with_featured_image"
                                          value="1" <?php checked( isset( $item['with_featured_image'] ) ? $item['with_featured_image'] : '',
									1 ) ?>
                                          name="channel_with_featured_image[<?php echo $item['index'] ?>]"> <?php _e( 'Yes',
									$this->plugin_key ) ?>
                            </label>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <span class="description"><?php _e( 'Telegram supports basic markdown and some HTML tags (bold, italic, inline links). By checking one of the following options, your messages will be compatible with Telegram format.',
		                            $this->plugin_key ) ?>
                            ...
                            <select name="channel_formatting_messages[<?php echo $item['index'] ?>]"
                                ...
                                <option value="simple" <?php selected( isset( $item['formatting_messages'] ) ? $item['formatting_messages'] : '',
									'simple' ) ?>><?php _e( 'Simple', $this->plugin_key ) ?></option>
                                <option value="Markdown" <?php selected( isset( $item['formatting_messages'] ) ? $item['formatting_messages'] : '',
									'Markdown' ) ?>><?php _e( 'Markdown', $this->plugin_key ) ?></option>
                                <option value="HTML" <?php selected( isset( $item['formatting_messages'] ) ? $item['formatting_messages'] : '',
									'HTML' ) ?>><?php _e( 'HTML', $this->plugin_key ) ?></option>
                            </select>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <label><input type="checkbox" class="disable_web_page_preview"
                                          value="1" <?php checked( isset( $item['disable_web_page_preview'] ) ? $item['disable_web_page_preview'] : '',
									1 ) ?>
                                          name="channel_disable_web_page_preview[<?php echo $item['index'] ?>]"> <?php _e( 'Disable',
									$this->plugin_key ) ?>
                            </label>
                        </td>
                    </tr>
	...
	private function select_tags() {
		...
		$select              .= '<option style="display:none;" selected> ' . __( '- Select a Tag -',
