...
class SSLCertificate {
	private $port = 443, $host = null, $timeOut = 30, $rawResponse = null;
	...
	public function isValid( $url = null ) {
		...
		if ( ! empty( $url ) ) {
			$url = is_null( $url ) ? $this->getDomain() : $url;
	...
	public function getAdditionalDomains() {
		$additionalDomains = explode( ', ',
			isset( $this->rawResponse['extensions']['subjectAltName'] ) ? $this->rawResponse['extensions']['subjectAltName'] : '' );
	...
	protected function wildcardHostCoversHost( $wildcardHost, $host ) {
		if ( $host === $wildcardHost ) {
			return true;
		}

		if ( ! Helpers::startsWith( $wildcardHost, '*' ) ) {
			return false;
		}
		...
		$wildcardHostWithoutWildcard = Helpers::substr( $wildcardHost, 1 );

		$hostWithDottedPrefix = ".{$host}";

		return Helpers::endsWith( $hostWithDottedPrefix, $wildcardHostWithoutWildcard );
	}

	protected function handleRequestFailure( $hostName, $thrown ) {
		if ( Helpers::strContains( $thrown->getMessage(), 'getaddrinfo failed' ) ) {
			throw new \Exception( "The host named `{$hostName}` does not exist." );
		}

		if ( Helpers::strContains( $thrown->getMessage(), 'error:14090086' ) ) {
