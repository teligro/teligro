...
class Debugs extends Teligro {
	...
	public function __construct() {
		...
		add_filter( 'teligro_debugs_info', [ $this, 'teligro_info' ], 1 );
		add_filter( 'teligro_debugs_info', [ $this, 'php_info' ] );
		add_filter( 'teligro_debugs_info', [ $this, 'wp_info' ] );
		add_filter( 'teligro_debugs_info', [ $this, 'host_info' ] );
		add_filter( 'teligro_debugs_info', [ $this, 'ssl_info' ] );
	}

	function admin_enqueue_scripts() {
		global $pagenow;
		if ( $pagenow == 'admin.php' && isset( $_GET['page'] ) && $_GET['page'] == 'teligro-debugs' ) {
	...
	function menu() {
		add_submenu_page( $this->plugin_key, $this->plugin_name . $this->page_title_divider . $this->page_title,
			$this->page_title, 'manage_options', $this->page_key, array( $this, 'pageContent' ) );
	}
	...
	function host_info( $debugs ) {
		// Host Info
		$domainInfo = new DomainInfo( $this->url );
		...
		if ( $domainIP ) {
			...
			$hostInfo      = array(
				'IP'                    => $domainIP,
				__( 'Host Location',
					$this->plugin_key ) => "<span class='ltr-right flex'><img src='https://www.countryflags.io/{$countryCode}/flat/16.png' alt='{$domainCountry['countryName']} Flag'> &nbsp;" . $domainCountry['countryCode'] . ' - ' . $domainCountry['countryName'] . '</span>'
			);
			if ( in_array( $domainCountry['countryCode'], $this->telegramFilterCountry ) )
				$hostInfo[ __( 'Tip',
					$this->plugin_key ) ] = __( 'Your website host location on the list of countries that have filtered the telegram. For this reason, the plugin may not work well. My suggestion is to use a host of another countries.',
					$this->plugin_key );
	...
	function ssl_info( $debugs ) {
		...
		if ( is_ssl() ) {
			...
			if ( is_array( $info ) ) {
				...
				$ssl_info[ __( 'Valid', $this->plugin_key ) ]  = $info['isValid'] ? __( 'Yes',
					$this->plugin_key ) : __( 'No', $this->plugin_key );

				$validFromDate            = Helpers::localeDate( $info['validFromDate'] );
				$ssl_info[ __( 'Valid from',
					$this->plugin_key ) ] = "<span class='ltr-right'>" . $info['validFromDate'] . ( $info['validFromDate'] != $validFromDate ? " / {$validFromDate}" : '' ) . "</span>";

				$expirationDate           = Helpers::localeDate( $info['expirationDate'] );
				$ssl_info[ __( 'Valid until',
					$this->plugin_key ) ] = "<span class='ltr-right'>" . $info['expirationDate'] . ( $info['expirationDate'] != $expirationDate ? " / {$expirationDate}" : '' ) . "</span>";

				$ssl_info[ __( 'Is expired', $this->plugin_key ) ]                   = $info['isExpired'] ? __( 'Yes',
					$this->plugin_key ) : __( 'No', $this->plugin_key );
	...
	function wp_info( $debugs ) {
		...
		if ( version_compare( $wp_version, '5.2', '>=' ) )
			$debugs[ __( 'WordPress' ) ][ __( 'Site Health',
				$this->plugin_key ) ] = '<a href="' . admin_url( 'site-health.php' ) . '">' . __( 'Check site health page',
					$this->plugin_key ) . '</a>';
	...
	function teligro_info( $debugs ) {
		...
		$debugs[ $this->plugin_name ] = array(
			__( 'Plugin Version', $this->plugin_key )          => TELIGRO_VERSION,
			__( 'Plugin DB Table Created', $this->plugin_key ) => $checkDBTable
		);

		if ( $updateInfo = $this->check_update_plugin() )
			$debugs[ $this->plugin_name ][ __( 'Update',
				$this->plugin_key ) ] = '<a href="' . $updateInfo['updateDetailURL'] . '" class="thickbox open-plugin-details-modal">' . __( 'Update to the new version',
					$this->plugin_key ) . ' ' . $updateInfo['newVersion'] . '</a>';
	...
	protected function check_update_plugin( $file = null ) {
		...
		if ( in_array( $file, array_keys( $plugin_updates ) ) )
			return array(
				...
				'updateURL'       => wp_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' ) . $file,
	...
	function pageContent() {
		$debugs = apply_filters( 'teligro_debugs_info', [] );
		?>
	...
	function checkSSLCertificate( $host ) {
		if ( ! is_ssl() || ! class_exists( 'teligro\SSLCertificate' ) )
			return false;
		try {
			$SSLCertificate = new SSLCertificate( $host );
